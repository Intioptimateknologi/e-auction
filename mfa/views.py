...
from eauctions import settings
...
class LoginView(DjangoLoginView):
    ...
    def post(self, request, *args, **kwargs):
        """Override post() to process reCAPTCHA and authenticate user."""
        ...
        recaptcha_response = requests.post(
            ...
            data={
                "secret": "6LfsDdgqAAAAAFR41pfRLdJdra05kt8rBGKqdTGg",
                "response": recaptcha_token,
            },
        ).json()
        print(recaptcha_response)
        if not recaptcha_response.get("success"):
            # return JsonResponse({"error": "reCAPTCHA validation failed."}, status=400)
            return redirect('/invalid-captcha/')
...
#            return JsonResponse({"success": "Login Success"}, status=200)  # Redirect on success
        else:

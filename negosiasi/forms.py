...
class verifikasi_revisi_penawaranForm(BSModalModelForm):
    class Meta:
        model = models.penawaran
        fields = [
            "bidder",
    ...
    def __init__(self, *args, **kwargs):
        super(verifikasi_revisi_penawaranForm, self).__init__(*args, **kwargs)
        if kwargs['initial'].get('id') is not None:
            undangan = models.revisi_penawaran.objects.get(pk=kwargs['initial']['id'])
            self.fields['bidder'] = forms.ModelChoiceField(
                queryset=bidder_user.objects.all().filter(id=undangan.bidder.id), 
                required=False, 
                widget=forms.Select)
...
class PenawaranForm(BSModalModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs['initial'].get('id') is not None:
            ...
            self.fields['bidder'] = forms.IntegerField(widget=forms.HiddenInput, initial=bidder_user.objects.all().get(users__id=self.request.user.id).id)
            obyek = models.obyek_seleksi_nego.objects.all().filter(bidder_user__users__id = self.request.user.id, item__item_lelang__id=kwargs['initial']['item'])
            o = []
            for j in obyek:
                o.append(j.item.id)
...
class RevisiPenawaranForm(BSModalModelForm):
    class Meta:
    ...
    # def __init__(self, *args, **kwargs):
    #     super(RevisiPenawaranForm, self).__init__(*args, **kwargs)
    #     if kwargs['initial'].get('id') is not None:
    #         undangan = models.revisi_penawaran.objects.get(pk=kwargs['initial']['id'])
    #         self.fields['bidder'] = forms.IntegerField(widget=forms.HiddenInput, initial=bidder_user.objects.all().get(users__id=self.request.user.id).id)
    #         self.fields['item'] = forms.ModelChoiceField(
    ...
    #         for j in obyek:
    #             o.append(j.item.id)
    #         exl = models.revisi_penawaran.objects.all().filter(bidder__users__id = self.request.user.id, item__item_lelang__id=kwargs['initial']['item'])
    #         e = []
    #         for ex in exl:
    #             e.append(ex.item.id)
    #         self.fields['item'] = forms.ModelChoiceField(
    ...
    # Yang original ada diatas, yang dibawah dimodif untuk mendapatkan itemlelang sesuai dengan data penawaran
    def __init__(self, *args, **kwargs):
        super(RevisiPenawaranForm, self).__init__(*args, **kwargs)
        if kwargs['initial'].get('id') is not None:
            undangan = models.revisi_penawaran.objects.get(pk=kwargs['initial']['id'])
            ...
            self.fields['bidder'] = forms.IntegerField(widget=forms.HiddenInput, initial=bidder_user.objects.all().get(users__id=self.request.user.id).id)
            obyek = models.penawaran.objects.all().filter(bidder__users__id = self.request.user.id, item__item_lelang__id=kwargs['initial']['item'])
            o = []
            for j in obyek:
                o.append(j.item.id)
            exl = models.revisi_penawaran.objects.all().filter(bidder__users__id = self.request.user.id, item__item_lelang__id=kwargs['initial']['item'])
...
class EvaluasiRevisiPenawaranForm(BSModalModelForm):
    class Meta:
    ...
    def __init__(self, *args, **kwargs):
        super(EvaluasiRevisiPenawaranForm, self).__init__(*args, **kwargs)
        if kwargs['initial'].get('id') is not None:
            undangan = models.evaluasi_revisi_penawaran.objects.get(pk=kwargs['initial']['id'])
            self.fields['revisi_penawaran'] = forms.ModelChoiceField(
                queryset=models.revisi_penawaran.objects.all().filter(id=undangan.revisi_penawaran.id), 
                required=False, 
                widget=forms.Select)
        else:
            obyek = models.revisi_penawaran.objects.all().filter(item__item_lelang__id=kwargs['initial']['item'])
            o = []
            for j in obyek:
                o.append(j.id)
            exl = models.evaluasi_revisi_penawaran.objects.all().filter(revisi_penawaran__item__item_lelang__id=kwargs['initial']['item'])
            e = []
            for ex in exl:
                e.append(ex.revisi_penawaran.id)
            self.fields['revisi_penawaran'] = forms.ModelChoiceField(
                queryset=models.revisi_penawaran.objects.all().filter(id__in = o).exclude(id__in=e), 
                required=False, 
                widget=forms.Select)
...
class obyek_seleksiForm(BSModalModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs['initial'].get('id') is not None:
            undangan = models.obyek_seleksi_nego.objects.get(pk=kwargs['initial']['id'])
